...
void SaveCFG(const char* FileName)
{
    ...
    char Path[0x100];
    sprintf(Path, ("C:\\NovazBesting\\"));

    if (!folder_exists(Path))
        _mkdir(Path);
    cfg.SetUnicode();
    char result[0x100];
    sprintf(result, ("C:\\NovazBesting\\cfg\\%s.ini"), FileName);
...
void LoadCFG(const char* FileName)
{
    ...
    char Path[0x100];
    sprintf(Path, "C:\\NovazBesting\\cfg\\{%lu}\\", SerialNumber);

    if (!folder_exists(Path))
        _mkdir(Path);
    cfg.SetUnicode();
    char result[0x100];
    sprintf(result, "C:\\NovazBesting\\cfg\\{%lu}\\%s.ini", SerialNumber, FileName);
    cfg.LoadFile(result);
