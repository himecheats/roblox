...
bool __fastcall fake_IsDown(void* inputstate, int button, void* method)
{
	...
	auto* TargetPlayer = reinterpret_cast<BasePlayer*>(Vars::Storage::closestPlayer);
	if (button == 1024 && TargetPlayer && visible && Vars::AimBot::AutoShoot)
	{
		return true;
	}
...
Vector3 __fastcall fake_EyePosition(void* eyes, void* method)
{
	...
	if (fake_eyes)
	{
		auto pos = (safe_read(cam, uintptr_t), StrW("position"), false);
		if (cam)
		{
			...
			safe_write(pos, real_eyes_pos, Vector3);
		}
...
{
	if (Vars::Misc::PeekAssist && Vars::Storage::closestPlayer)
	{
		printf("1\n");
		Vector3 startPos = (fake_eyes ? fake_eyes_pos : original_EyePosition((void*)safe_read(LocalPlayer.BasePlayer + 0x660, uintptr_t), method));
		Vector3 endPos = ((BasePlayer*)Vars::Storage::closestPlayer)->GetBoneByID(head);
		float d = endPos.y - startPos.y;
		Vector3 realPos = startPos;
		printf("1\n");
		typedef bool(__stdcall* V)(Vector3, Vector3, int, float, void*);
		V s = (V)(Vars::Storage::gBase + 6530624);
		printf("1\n");

		if (s(startPos, endPos, 10551296, 3.f, 0))
		{
			printf("1\n");
		...
		startPos.y = startPos.y + Vars::Misc::LongNeckvolue;
		if (s(startPos, endPos, 10551296, 4.f, 0))
		{
			//printf("(UP)\nRealPos: (%ff, %ff, %ff)\nStartPos: (%ff, %ff, %ff)\nEndPos: (%ff, %ff, %ff)\n---------------\n", realPos.x, realPos.y, realPos.z, startPos.x, startPos.y, startPos.z, endPos.x, endPos.y, endPos.z);
			fake_eyes = true;
			real_eyes_pos = realPos;
		...
		startPos.y = startPos.y - Vars::Misc::LongNeckvolue;
		startPos.x = startPos.x + Vars::Misc::sidepeekvalue;
		if (s(startPos, endPos, 10551296, 3.f, 0))
		{
			//printf("(LEFT)\nRealPos: (%ff, %ff, %ff)\nStartPos: (%ff, %ff, %ff)\nEndPos: (%ff, %ff, %ff)\n---------------\n", realPos.x, realPos.y, realPos.z, startPos.x, startPos.y, startPos.z, endPos.x, endPos.y, endPos.z);
		...
		startPos.x = startPos.x - (Vars::Misc::sidepeekvalue * 2.f);
		if (s(startPos, endPos, 10551296, 3.f, 0))
		{
			//printf("(RIGHT)\nRealPos: (%ff, %ff, %ff)\nStartPos: (%ff, %ff, %ff)\nEndPos: (%ff, %ff, %ff)\n---------------\n", realPos.x, realPos.y, realPos.z, startPos.x, startPos.y, startPos.z, endPos.x, endPos.y, endPos.z);
			//set player eyes location to startpos and keep playereyes camera at same position
...
inline void __fastcall CustomHitsound(DWORD64 info) {
	if (Vars::Misc::CustomHitsound) {
		...
		if (Vars::Misc::HitSound1) {
			PlaySoundW((L"C:\\NovazBesting\\Sound\\1.wav"), NULL, SND_ASYNC);
		}
...
inline void InitHook()
{
	...
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x63A440)), reinterpret_cast<void**>(&WLineOfSight), LineOfSight); // internal virtual void ClientInput(InputState state) { }
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x7F5D40)), reinterpret_cast<void**>(&original_LaunchProjectileClientside), fake_LaunchProjectileClientside);
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 7147568)), reinterpret_cast<void**>(&original_MainCamGetPos), fake_MainCamGetPos);
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x968F00)), reinterpret_cast<void**>(&original_EyePosition), fake_EyePosition);
	CreateHook(reinterpret_cast<void*>(static_cast<uintptr_t>(GetModBase(L"GameAssembly.dll") + 0x904D60)), reinterpret_cast<void**>(&original_IsDown), fake_IsDown); // internal virtual void ClientInput(InputState state) { }
