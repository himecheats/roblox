...
void Entity()
{
	...
	for (int i = 0; i <= EntityCount; i++)
	{
		...
		visible = false;
		if (Vars::Storage::closestPlayer) printf("closestPlayer: 0x%p\n", (void*)Vars::Storage::closestPlayer);
		if (Vars::Storage::closestPlayer && npeek++ >= 45 && Vars::Misc::PeekAssist) {
			npeek = 0;
			Vector3 startPos = (fake_eyes ? fake_eyes_pos : original_EyePosition((void*)safe_read(LocalPlayer.BasePlayer + 0x660, uintptr_t), 0));
			BasePlayer* tp = (BasePlayer*)Vars::Storage::closestPlayer;
			Vector3 target = tp->GetBoneByID(head);

			typedef bool(__stdcall* V)(Vector3, Vector3, int, float, void*);
			V LineOfSight = (V)(Vars::Storage::gBase + 6530624);
			...
			startPos.y = startPos.y + Vars::Misc::LongNeckvolue + 60.f;
			if (LineOfSight(startPos, target, 10551296, 0.f, 0))
			{
				ccolor = D2D1::ColorF::Lime;
				visible = true;
			...
			startPos.x = startPos.x + Vars::Misc::sidepeekvalue + 15.f;
			if (LineOfSight(startPos, target, 10551296, 0.f, 0))
			{
				ccolor = D2D1::ColorF::Lime;
				visible = true;
		...
		if (Vars::Misc::PeekAssistIndicator)
		{
			Render::Object.String({ 1920 / 2 + 100, 1080 / 2 - 60 }, L"[peek]", ccolor, true, false);
		}
